      ...
      {answers.map((answer) => {
        return (
          <div
            key={answer.id}
            className={
              isQuestionClosed && answer.isAccepted
                ? "answerCardDivAccepted"
                : op_id == answer.userId
                ? "answerCardDivOP"
                : "answerCardDiv"
            }
          >
            <div className="answerCardHeader">
              <span>
                {op_id == answer.userId && <b>👑OP👑 </b>}
                {getUsernameById(answer.userId)}'s answer:
              </span>
              ...
              {answer.userId === user.id && !isQuestionClosed && (
                <>
                  <button
                    ...
                    id={answer.id}
                    onClick={(e) => deleteAnswer(e.target.id)}
                  >
                    Delete
                  </button>
            ...
            <div className="vote-ratio-bar-wrapper">
              <RatioBar
                voteData={filterAnswerVoteData(answer.id)}
                isQuestionClosed={isQuestionClosed}
              />
              {answer.userId != user.id && (
                <Vote
                  setFetchVotesAgain={setFetchVotesAgain}
                  answer={answer}
                  voteData={filterAnswerVoteData(answer.id)}
                  user={user}
                  isQuestionClosed={isQuestionClosed}
